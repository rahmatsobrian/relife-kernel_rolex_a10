                           ...
                           struct iw_point *dwrq, char *extra)
{
    ...
    if (pRoamProfile->Keys.KeyLength[keyId] > 0)
    {
        ...
        vos_mem_copy(extra,
                     &(pRoamProfile->Keys.KeyMaterial[keyId][0]),
    ...
    // Perbaikan: jangan cek alamat array, tapi panjang key-nya
   ...
   if(wrqu->data.flags & IW_ENCODE_DISABLED)
   {
       ...
       if(!fKeyPresent) {
          ...
          for(i=0;i < CSR_MAX_NUM_KEY; i++) {
             ...
             if (pWextState->roamProfile.Keys.KeyLength[i] > 0)
               ...
               char *extra)
{
    ...
    {
        if (pRoamProfile->Keys.KeyLength[i] == 0) {
    continue;  // Kunci tidak ada, lewati iterasi
}
        else
        {
            break;
                                                  ...
                                                  (IW_ENCODE_ALG_WEP == ext->alg))
    {
       if(IW_AUTH_KEY_MGMT_802_1X == pWextState->authKeyMgmt) {
         ...
         /*Static wep, update the roam profile with the keys */
          if (ext->key_len > 0 && ext->key_len <= eCSR_SECURITY_WEP_KEYSIZE_MAX_BYTES && key_index < CSR_MAX_NUM_KEY) {
