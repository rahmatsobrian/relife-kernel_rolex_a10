            ...
            struct diag_cmd_reg_entry_t *entry, int proc)
{
    ...
    list_for_each_safe(start, temp, &driver->cmd_reg_list) {
        ...
        // Memeriksa apakah entry valid berdasarkan properti di dalamnya
        if (temp_entry->cmd_code == 0 && temp_entry->subsys_id == 0) {
...
void diag_cmd_remove_reg_by_pid(int pid)
{
	...
	list_for_each_safe(start, temp, &driver->cmd_reg_list) {
		item = list_entry(start, struct diag_cmd_reg_t, link);
		if (!item) {
    pr_err("diag: In %s, invalid list item\n", __func__);
    mutex_unlock(&driver->cmd_reg_mutex);
    return;
}
		if (item->pid == pid) {
